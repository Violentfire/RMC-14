rmc-chem-master-full-pill-bottles = Машина повністю завантажена пляшечками з пігулками.

rmc-chem-master-status = [bold]Статус[/bold]
rmc-chem-master-reagent-amount = {$name}, {$amount} одиниць
rmc-chem-master-all = Все
rmc-chem-master-custom = Кастомне
rmc-chem-master-all-reagents = Всі речовини

rmc-chem-master-beaker = Колба:
rmc-chem-master-beaker-none = Колба відсутня.
rmc-chem-master-beaker-amount = {$amount} од.

rmc-chem-master-pill-bottle = Пляшки Пігулок:
rmc-chem-master-pill-bottle-none = Пляшки пігулок відсутні.
rmc-chem-master-pill-bottle-fill = Заповнити
rmc-chem-master-pill-bottle-pills = {$amount} / {$total}
rmc-chem-master-pill-bottle-transfer = Скласти
rmc-chem-master-pill-bottle-eject = Діст.
rmc-chem-master-pill-bottle-window-title = Колір пляшечки

rmc-chem-master-pill-bottle-box-empty = {$box} is empty and cannot be unloaded into the ChemMaster.
rmc-chem-master-pill-bottle-box-start = You start to empty the {$box} into the {$target}...
rmc-chem-master-pill-bottle-box-complete = You transferred {$count} pill bottles into the {$target}.
rmc-chem-master-pill-bottle-box-failed = Failed to transfer any pill bottles.

rmc-chem-master-beaker-title = [bold]Колба[/bold]
rmc-chem-master-beaker-eject = Вийняти колбу
rmc-chem-master-beaker-empty = Колба порожня.

rmc-chem-master-buffer = [bold]Буфер[/bold]
rmc-chem-master-buffer-to-beaker = До буферу
rmc-chem-master-buffer-to-disposal = На утилізацію
rmc-chem-master-buffer-empty = Буфер порожній.

rmc-chem-master-pills = [bold]Пігулки[/bold]
rmc-chem-master-pills-create = Створити пігулки
rmc-chem-master-pills-not-enough-space = У вибраних пляшечках для пігулок недостатньо місця!

rmc-chem-master-glassware = [bold]Скляний посуд[/bold]
rmc-chem-master-create-bottle = Створити пляшку ({$amount} од.)
rmc-chem-master-create-transfer-bottle = Створити та перенести

rmc-chem-master-not-enough-space-solution = У буфері недостатньо хімікатів!